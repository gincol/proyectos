...
@Lazy
@Service(ServicesConstants.SRV_FRONT_VALIDATE_SERVICE_EVENT_FMW)
public class ValidatorServiceEvent {
	...
	public ReturnRDTO validateInsert(EventServiceRDTO eventRDTO) {
		...
		if (isRequestValid && !validator.validateExistsItineraryGap(eventRDTO.getCodeItinerary())){
			...
			isMessage	= String.format(ReturnEnum.CODE_ERROR_ATTRIBUTE_NOT_FOUND.getMessage(), 
					...
					MessageContansts.MSG_ENTITY_SERVICE_EVENT,
		...
		if (isRequestValid && !validator.validateExistsGroupEventType(ValidationConstants.GROUP_SERVICE_EVENT, eventRDTO.getCodeType())) {
			isRequestValid = false;
		...
		//motiu
		if (isRequestValid && !validator.validateValueListGap(eventRDTO.getCodeReason(),
				new Entity(ValueListConstants.CODE_TYPE_REASON))) {
			isRequestValid = false;
			isCode = ReturnEnum.CODE_ERROR_ATTRIBUTE_NOT_FOUND.getCodeDescription();
			isMessage = String.format(ReturnEnum.CODE_ERROR_ATTRIBUTE_NOT_FOUND.getMessage(),
					MessageContansts.MSG_FIELD_REASON,
					MessageContansts.MSG_ENTITY_SERVICE_EVENT, eventRDTO.getCodeReason());
		}
		//motiu
		if (isRequestValid && !validator.validateValueListGap(eventRDTO.getCodeSubReason(),
				new Entity(ValueListConstants.CODE_TYPE_SUBREASON))) {
			isCode = ReturnEnum.CODE_ERROR_ATTRIBUTE_NOT_FOUND.getCodeDescription();
			isMessage = String.format(ReturnEnum.CODE_ERROR_ATTRIBUTE_NOT_FOUND.getMessage(),
					MessageContansts.MSG_FIELD_SUBREASON,
					MessageContansts.MSG_ENTITY_SERVICE_EVENT, eventRDTO.getCodeSubReason());
		}
