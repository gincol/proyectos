...
@Lazy
@Service(ServicesConstants.SRV_FRONT_VALIDATE_ANNULMENT_EVENT_FMW)
public class ValidatorAnnulmentEvent {
	...
	public ReturnRDTO validateInsert(AnnulmentEventRDTO eventRDTO) {
		...
		if (isRequestValid && !validator.validateExistsItineraryGap(eventRDTO.getCodeItinerary())){
			...
			isMessage	= String.format(ReturnEnum.CODE_ERROR_ATTRIBUTE_NOT_FOUND.getMessage(), 
					...
					MessageContansts.MSG_ENTITY_ANNULMENT_EVENT,
		...
		//event annuled
		if (isRequestValid && !StringUtils.isEmpty(eventRDTO.getCodeAnnulled()) && !validator.validateExistsEventGap(eventRDTO.getCodeAnnulled())){
			isRequestValid = false;	
			isCode 		= ReturnEnum.CODE_ERROR_ATTRIBUTE_NOT_FOUND.getCodeDescription();
			isMessage	= String.format(ReturnEnum.CODE_ERROR_ATTRIBUTE_NOT_FOUND.getMessage(), 
					MessageContansts.MSG_FIELD_CODE_ANNULLED,
					MessageContansts.MSG_ENTITY_ANNULMENT_EVENT,
					eventRDTO.getCodeAnnulled());
		}
		//tipo evento
		...
		if (isRequestValid && !validator.validateExistsGroupEventType(ValidationConstants.GROUP_ANULLMENT_EVENT, eventRDTO.getCodeType())) {
			isRequestValid = false;
		...
		if (isRequestValid && !StringUtils.isEmpty(eventRDTO.getCodeReason()) && !validator.validateValueListGap(eventRDTO.getCodeReason(),
			...
			isMessage = String.format(ReturnEnum.CODE_ERROR_ATTRIBUTE_NOT_FOUND.getMessage(),
					...
					MessageContansts.MSG_ENTITY_ANNULMENT_EVENT, eventRDTO.getCodeReason());
		}
		...
		if (isRequestValid && !StringUtils.isEmpty(eventRDTO.getCodeSubReason()) && !validator.validateValueListGap(eventRDTO.getCodeSubReason(),
			...
			isMessage = String.format(ReturnEnum.CODE_ERROR_ATTRIBUTE_NOT_FOUND.getMessage(),
					...
					MessageContansts.MSG_ENTITY_ANNULMENT_EVENT, eventRDTO.getCodeSubReason());
		}		
