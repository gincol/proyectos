...
@Lazy
@Service(ServicesConstants.SRV_FRONT_VALIDATE_UNLOADING_EVENT_FMW)
public class ValidatorUnloadingEvent {
	...
	public ReturnRDTO validateInsert(UnloadingEventRDTO eventRDTO) {
		...
		if (isRequestValid && !validator.validateExistsItineraryGap(eventRDTO.getCodeItinerary())){
			...
			isMessage	= String.format(ReturnEnum.CODE_ERROR_ATTRIBUTE_NOT_FOUND.getMessage(), 
					...
					MessageContansts.MSG_ENTITY_UNLOADING_EVENT,
		...
		if (isRequestValid && !validator.validateExistsGroupEventType(ValidationConstants.GROUP_UNLOADING_EVENT, eventRDTO.getCodeType())) {
			isRequestValid = false;
		...
		if (isRequestValid && !StringUtils.isEmpty(eventRDTO.getCodeReason()) &&!validator.validateValueListGap(eventRDTO.getCodeReason(),
			...
			isMessage = String.format(ReturnEnum.CODE_ERROR_ATTRIBUTE_NOT_FOUND.getMessage(),
					...
					MessageContansts.MSG_ENTITY_UNLOADING_EVENT, eventRDTO.getCodeReason());
		}
		//submotiu
		if (isRequestValid && !StringUtils.isEmpty(eventRDTO.getCodeSubReason()) && !validator.validateValueListGap(eventRDTO.getCodeSubReason(),
			...
			isMessage = String.format(ReturnEnum.CODE_ERROR_ATTRIBUTE_NOT_FOUND.getMessage(),
					...
					MessageContansts.MSG_ENTITY_UNLOADING_EVENT, eventRDTO.getCodeSubReason());
		}
		

		//planta psseatge
		if (isRequestValid && !StringUtils.isEmpty(eventRDTO.getCodePlantWeighing()) && !validator.validateExistsPlantWeighingGap(eventRDTO.getCodePlantWeighing())) {
			isCode = ReturnEnum.CODE_ERROR_ATTRIBUTE_NOT_FOUND.getCodeDescription();
			isMessage = String.format(ReturnEnum.CODE_ERROR_ATTRIBUTE_NOT_FOUND.getMessage(),
					MessageContansts.MSG_FIELD_PLANT_WEIGHING,
					MessageContansts.MSG_ENTITY_UNLOADING_EVENT, eventRDTO.getCodePlantWeighing());
		}
