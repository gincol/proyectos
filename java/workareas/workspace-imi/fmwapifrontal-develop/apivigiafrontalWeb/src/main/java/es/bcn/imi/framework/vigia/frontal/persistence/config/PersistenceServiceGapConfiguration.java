package es.bcn.imi.framework.vigia.frontal.persistence.config;
...
@EntornPropertySource(value = {
		"classpath:/applibgap/mybatis/config/persistence.properties" })
@Lazy(true)
public class PersistenceServiceGapConfiguration {
	...
	@ConditionalOnContext(property = net.opentrends.openframe.services.persistence.util.Constants.CONTEXT_CONFIGURATION_SERVICE_PROPERTY_NAME,
		value = net.opentrends.openframe.services.configuration.util.Constants.CONTEXT_CONFIGURATION_FWK_MOCK_MODE)
	...
	@Bean(name = "dataSourceGap")
	@ConditionalOnContext(property = net.opentrends.openframe.services.persistence.util.Constants.CONTEXT_CONFIGURATION_SERVICE_PROPERTY_NAME,
		value = ConditionalOnContext.NOT_EXISTS_OR_IS_EMPTY)
	...
	private DataSource dataSourceJndiImpl() {
		...
		Assert.hasText(dataSourceName, "The 'persistence.dataSource.jndiName' placeholder must have a non empty value.");
	...
	private DataSource dataSourceOracleJdbcPoolImpl() throws PersistenceServiceException {
		...
		Assert.notNull(settings, "A not null JdbcSettings is required for instantiating a dataSource (Oracle JDBC Pool version)");
		Assert.hasText(settings.getUrl(), "The URL is required for instantiating a dataSource (Oracle JDBC Pool version)");
		Assert.hasText(settings.getUsername(), "The username is required for instantiating a dataSource (Oracle JDBC Pool version)");
		Assert.hasText(settings.getPassword(), "The password is required for instantiating a dataSource (Oracle JDBC Pool version)");
		try {
	...
	/**
	 * Specify the settings required to established a JDBC connection during
	 * the execution of Batch processes.
	 * 
	...
	@ConditionalOnContext(property = net.opentrends.openframe.services.persistence.util.Constants.CONTEXT_CONFIGURATION_SERVICE_PROPERTY_NAME,
			value = ConditionalOnContext.NOT_EXISTS_OR_IS_EMPTY,
			otherConditions = { BatchProcessCondition.class },
			otherConditionsMatching = ConditionsMatching.ALL)
	...
	protected SqlSessionFactoryBean sqlSessionFactoryBean() throws PersistenceServiceException {
		...
		SqlSessionFactoryBeanBuilder builder = 
				new SqlSessionFactoryBeanBuilder(
						dataSource(),
						myBatisMapper);

		if (myBatisMapperAdditionalMappingFilesFolderLocations != null && myBatisMapperAdditionalMappingFilesFolderLocations.length > 0) {
	...
	/**
	 * Gives inherited configuration classes the opportunity to add further
	 * MyBatis plugins.
	 * 
	 * @return
	 */
	...
	/**
	 * Gives inherited configuration classes (and also those configuration
	 * classes that import this class instead of inheriting it) the opportunity
	 * to provide the location of a MyBatis {@code SqlSessionFactory}
	 * configuration file for configuring the MyBatis {@link SqlSession}.
	 * 
	 * @return any implementation of
	 * 		{@link org.springframework.core.io.Resource}, typically a
	 * 		{@link org.springframework.core.io.ClassPathResource} initialized
	 * 		with "/app/mybatis/mybatis-configuration.xml".
	 */
	...
	/**
	 * As an alternative to the {@code myBatisConfigLocation()} method,
	 * gives inherited configuration classes (and also those configuration
	 * classes that import this class instead of inheriting it) the opportunity
	 * to provide a customized MyBatis Configuration for configuring the
	 * MyBatis {@link SqlSession}.
	 * 
	 * @return a customized MyBatis
	 * 		{@link org.apache.ibatis.session.Configuration} object.
	 */
